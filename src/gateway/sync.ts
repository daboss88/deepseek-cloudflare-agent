...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ ; rsync -r --no-times --delete --exclude='node_modules' /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ ; rsync -r --no-times --delete /root/clawd/agent/ ${R2_MOUNT_PATH}/agent/ ; date -Iseconds > ${R2_MOUNT_PATH}/.last-sync ; HOUR=$(TZ=Asia/Kuala_Lumpur date +%H) ; if [ "$HOUR" = "07" ]; then if [ ! -f /tmp/b_sent ]; then node /root/clawd/morning-briefing.js ; touch /tmp/b_sent; fi; else rm -f /tmp/b_sent; fi`;
